...
- **THEN** an edit modal SHALL open showing:
  - The key name
  - A label indicating this is editing the original text (e.g. 「編輯原文」/「Editing Original Text」/「원문 편집」)
  - A textarea pre-filled with the current English source text
- **AND** the meta subtitle SHALL display `{key} — {editOriginalLabel}` where `{editOriginalLabel}` is the i18n string for "Editing Original Text" in the current UI language
- **AND** the source reference block SHALL NOT be displayed
...
- **THEN** the edit modal SHALL open directly (no confirmation needed)
