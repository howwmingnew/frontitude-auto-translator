...
#### Scenario: Reverted cell loses edited highlight
- **WHEN** a cell has been reverted to its original value via the revert button
- **THEN** that cell SHALL NOT display the edited-cell highlight color
- **AND** the highlight removal SHALL be visible immediately after the table re-renders
